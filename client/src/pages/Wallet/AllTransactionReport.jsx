...
const AllTransactionReport = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                console.error('Error fetching all transaction report:', error);
                setTransactions([]);
            } finally {
                setLoading(false);
            }
    ...
    }, [transactions, fromDate, searchMode, toDate]);
            ...
            <div className="mx-auto max-w-[1280px]">
                <div className="mb-5 hidden items-start justify-start md:mb-6 md:flex md:justify-end">
                    <div className="min-w-0 text-left md:text-right">
                        <h1 className="break-words text-[1.45rem] font-black leading-tight text-[#F5E6C8] sm:text-[1.7rem] md:text-[2.05rem]">All Transaction Report</h1>
                    ...
                    <div className="overflow-hidden rounded-[2px] border border-[#C8A96A]/20 bg-[#171717] shadow-[0_14px_40px_rgba(0,0,0,0.35)]">
                        ...
                        <form onSubmit={(event) => { event.preventDefault(); setSearched(true); }} className="p-3 sm:p-6">
                            <div className="flex flex-col gap-3 text-[12px] text-[#F5E6C8]/75 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
                                <label className="inline-flex items-center gap-2"><input type="radio" name="txnSearch" checked={searchMode === 'all'} onChange={() => setSearchMode('all')} accentColor="#C8A96A" />All Record</label>
                                <label className="inline-flex items-center gap-2"><input type="radio" name="txnSearch" checked={searchMode === 'between'} onChange={() => setSearchMode('between')} accentColor="#C8A96A" />Between Dates</label>
                            </div>
                            {searchMode === 'between' && (
                ...
                <div className="mt-4 overflow-hidden rounded-[2px] border border-[#C8A96A]/20 bg-[#171717] shadow-[0_14px_40px_rgba(0,0,0,0.35)] md:mt-10">
                    <div className="flex flex-col gap-3 bg-[#1F1F1F] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                        <span className="break-words text-[11px] font-black uppercase tracking-[0.18em] text-[#C8A96A]">All Transaction Report</span>
                        <span className="w-fit rounded-[2px] bg-[#C8A96A] px-2 py-1 text-[10px] font-bold text-[#0D0D0D]">{filteredTransactions.length} Records</span>
                    </div>
                    <div className="p-4">
                        {loading ? <div className="py-10 text-center text-sm text-[#F5E6C8]/55">Loading report...</div> : filteredTransactions.length === 0 ? <div className="py-10 text-center text-sm text-[#F5E6C8]/55">{searched ? 'No transaction records found for selected dates.' : 'No transaction records available.'}</div> : (
                            <>
                                ...
                                <div className="hidden overflow-x-auto overscroll-x-contain md:block" style={{ WebkitOverflowScrolling: 'touch', touchAction: 'pan-x' }}>
                                    <table className="w-full min-w-[920px] border-collapse text-left text-sm">
                                        <thead><tr className="bg-[#111111] text-[#C8A96A]">
                                            ...
                                            <th className="border border-[#C8A96A]/15 px-3 py-2">Tx Type</th>
                                        ...
                                        <tbody>{filteredTransactions.map((item) => (
                                            <tr key={item._id} className="bg-[#171717] text-[#F5E6C8]/90">
                                                ...
                                                <td className="border border-[#C8A96A]/10 px-3 py-2 uppercase">{item.txType || '-'}</td>
                                                ...
                                                <td className={`border border-[#C8A96A]/10 px-3 py-2 ${item.txType === 'credit' ? 'text-green-400' : 'text-red-400'}`}>{item.txType === 'credit' ? '+' : '-'}Rs {formatAmount(item.amount)}</td>
                                            </tr>
                                        ))}</tbody>
                                    </table>
                                </div>
