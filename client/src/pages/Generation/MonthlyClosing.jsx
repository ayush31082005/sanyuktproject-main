...
const MonthlyClosing = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                console.error('Error fetching monthly closing report:', error);
                setTransactions([]);
            } finally {
                setLoading(false);
            }
    ...
    const monthlyRows = useMemo(() => {
        ...
        filteredTransactions.forEach((item) => {
            ...
            if (item.txType === 'credit') {
                current.credits += Number(item.amount || 0);
            } else {
                current.debits += Number(item.amount || 0);
            }
            current.entries += 1;
        });
    ...
    const totalAmount = monthlyRows.reduce((sum, item) => sum + item.net, 0);
