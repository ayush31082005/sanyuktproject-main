    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                console.error('Error fetching generation deduction report:', error);
                setDeductions([]);
            } finally {
                setLoading(false);
            }
                        ...
                        >
                            <div className="flex flex-col gap-3 text-[12px] text-[#F5E6C8]/75 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
                                <label className="inline-flex items-center gap-2">
                                    <input type="radio" name="generationDeductionSearch" checked={searchMode === 'all'} onChange={() => setSearchMode('all')} accentColor="#C8A96A" />
                                    All Record
                                </label>
                                <label className="inline-flex items-center gap-2">
                                    <input type="radio" name="generationDeductionSearch" checked={searchMode === 'between'} onChange={() => setSearchMode('between')} accentColor="#C8A96A" />
                                    Between Dates
                                </label>
                            </div>
                            {searchMode === 'between' && (
                    ...
                    <div className="p-4">
                        {loading ? (
                            ...
                            <div className="py-10 text-center text-sm text-[#F5E6C8]/55">
                                {searched ? 'No deduction records found for selected dates.' : 'No deduction records available.'}
                            </div>
                        ) : (
                            <>
